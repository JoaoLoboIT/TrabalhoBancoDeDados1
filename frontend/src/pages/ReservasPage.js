...
import {
  Typography, Container, List, ListItem, ListItemText, ListItemSecondaryAction,
  CircularProgress, Alert, Paper, Grid, TextField, Box, Chip,
  Autocomplete, Button, Dialog, DialogTitle, DialogContent, DialogActions, IconButton
} from '@mui/material';
...
function ReservasPage() {
    ...
    const fetchInitialData = async () => {
      try {
        const [usuariosRes, espacosRes] = await Promise.all([
          fetch('http://localhost:5000/api/usuarios', { headers: { 'x-access-token': token } }),
  ...
  const handleCancel = (reservaId) => {
    if (window.confirm('Tem certeza que deseja cancelar esta reserva?')) {
      ...
      .then(res => res.json().then(data => {
        if (!res.ok) throw new Error(data.erro || 'Falha ao cancelar reserva.');
        fetchReservas();
      }))
      .catch(err => {
        setError(err.message);
      ...
      {!loading && !error && (
        <List>
          {reservas.length > 0 ? reservas.map((reserva) => (
            <ListItem key={reserva.reserva_id} divider
              secondaryAction={
            ...
            >
              <ListItemText
                primary={`${reserva.espaco_nome} - ${reserva.finalidade || "Sem finalidade"}`}
                secondary={
                  <>
                    <Typography component="span" variant="body2" display="block">Solicitante: {reserva.solicitante_nome}</Typography>
                    <Typography component="span" variant="body2" display="block">Início: {new Date(reserva.data_hora_inicio).toLocaleString("pt-BR")}</Typography>
                    <Typography component="span" variant="body2" display="block">Fim: {new Date(reserva.data_hora_fim).toLocaleString("pt-BR")}</Typography>
                  </>
                }
              />
              <Chip label={reserva.status} color={getStatusChipColor(reserva.status)} />
            </ListItem>
